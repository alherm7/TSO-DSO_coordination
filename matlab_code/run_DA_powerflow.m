function [result_DAflow_PCC, result_DAflow_FC, result_DAflow_CM] ...
...
disp([' '])
disp([' '])
disp([' Fixed DA-dispatch Power Flow '])
...
OOS_time = tic;
[result_DAflow_PCC] = DA_powerflow(mpc,DA_outcome_PCC,kk);
[result_DAflow_CM] = DA_powerflow(mpc,DA_outcome_CM,kk);
[result_DAflow_FC] = DA_powerflow(mpc,DA_outcome_FC,kk);
tfin_OOS = toc(OOS_time);

disp(['WPP-DAflow: ' num2str(kk) ', Time: ' num2str(tfin_OOS)])
